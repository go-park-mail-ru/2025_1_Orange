...
func (gw *Gateway) Logout(ctx context.Context, session string) error {
	timer := prometheus.NewTimer(metrics.ExternalCallDuration.WithLabelValues("auth_mc", "Logout"))
	defer timer.ObserveDuration()

	_, err := gw.authClient.Logout(ctx, &authPROTO.LogoutRequest{Session: session})
	if err != nil {
		metrics.ExternalCallCounter.WithLabelValues("auth_mc", "Logout", "500").Inc()
		return err
	}

	metrics.ExternalCallCounter.WithLabelValues("auth_mc", "Logout", "200").Inc()
	return nil
}

func (gw *Gateway) LogoutAll(ctx context.Context, userID int, role string) error {
	timer := prometheus.NewTimer(metrics.ExternalCallDuration.WithLabelValues("auth_mc", "LogoutAll"))
	defer timer.ObserveDuration()

	_, err := gw.authClient.LogoutAll(ctx, &authPROTO.LogoutAllRequest{UserId: uint64(userID), Role: role})
	if err != nil {
		metrics.ExternalCallCounter.WithLabelValues("auth_mc", "LogoutAll", "500").Inc()
		return err
	}

	metrics.ExternalCallCounter.WithLabelValues("auth_mc", "LogoutAll", "200").Inc()
	return nil
}

func (gw *Gateway) GetUserIDBySession(ctx context.Context, session string) (int, string, error) {
	timer := prometheus.NewTimer(metrics.ExternalCallDuration.WithLabelValues("auth_mc", "GetUserIDBySession"))
	defer timer.ObserveDuration()

	resp, err := gw.authClient.GetUserIDBySession(ctx, &authPROTO.GetUserIDBySessionRequest{Session: session})
	if err != nil {
		metrics.ExternalCallCounter.WithLabelValues("auth_mc", "GetUserIDBySession", "500").Inc()
		return -1, "", err
	}

	metrics.ExternalCallCounter.WithLabelValues("auth_mc", "GetUserIDBySession", "200").Inc()
...
func (gw *Gateway) CreateSession(ctx context.Context, userID int, role string) (string, error) {
	timer := prometheus.NewTimer(metrics.ExternalCallDuration.WithLabelValues("auth_mc", "CreateSession"))
	defer timer.ObserveDuration()

	resp, err := gw.authClient.CreateSession(ctx, &authPROTO.CreateSessionRequest{UserId: uint64(userID), Role: role})
	if err != nil {
		metrics.ExternalCallCounter.WithLabelValues("auth_mc", "CreateSession", "500").Inc()
		return "", err
	}
	metrics.ExternalCallCounter.WithLabelValues("auth_mc", "CreateSession", "200").Inc()
