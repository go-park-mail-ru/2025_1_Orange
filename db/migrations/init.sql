...
(
    id              INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    company_name            TEXT
        CONSTRAINT company_name_length CHECK (LENGTH(company_name) <= 64) NOT NULL UNIQUE,
    slogan          TEXT,
    website         TEXT
...
(
    id                    INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    title                 TEXT
        CONSTRAINT title_length CHECK (LENGTH(title) <= 64) NOT NULL,
...
-- Индексы для таблицы applicant
CREATE INDEX idx_applicant_city ON applicant(city_id);
CREATE INDEX idx_applicant_avatar ON applicant(avatar_id);
CREATE INDEX idx_applicant_status ON applicant(status);

-- Индексы для таблицы employer
CREATE INDEX idx_employer_logo ON employer(logo_id);

-- Индексы для таблицы user_info_link
CREATE INDEX idx_user_info_link_image ON user_info_link(image_id);
CREATE INDEX idx_user_info_link_composite ON user_info_link(user_type, user_id);

-- Индексы для таблицы vacancy_city
CREATE INDEX idx_vacancy_city_city ON vacancy_city(city_id);

-- Индексы для таблицы vacancy_supplementary_conditions
CREATE INDEX idx_vacancy_supp_cond ON vacancy_supplementary_conditions(condition_id);

-- Индексы для таблицы vacancy_skill
CREATE INDEX idx_vacancy_skill_skill ON vacancy_skill(skill_id);

-- Индексы для таблицы work_experience
CREATE INDEX idx_work_experience_resume ON work_experience(resume_id);

-- Индексы для таблицы resume_specialization
CREATE INDEX idx_resume_spec_specialization ON resume_specialization(specialization_id);

-- Индексы для таблицы resume_skill
CREATE INDEX idx_resume_skill_skill ON resume_skill(skill_id);

-- Индексы для таблицы vacancy_response
CREATE INDEX idx_vacancy_response_applicant ON vacancy_response(applicant_id);

-- Индексы для таблицы vacancy_like
CREATE INDEX idx_vacancy_like_applicant ON vacancy_like(applicant_id);